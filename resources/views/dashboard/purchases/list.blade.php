...
<script type="text/javascript">
       ...
       $('#tbl_list').DataTable({
            ...
            columns: [
                ...
                {data: null, name: 'action', orderable: false, searchable: false, render: function(data, type, full, meta){
                    $output = "<a href=\"{{ route('purchases.view') }}/" + full.id + "\" class=\"hover:text-primary\" title=\"View Purchases\"><i class=\"fi fi-rr-eye\"></i></a>&nbsp;&nbsp;";
                    $output += "<a href=\"{{ route('purchases.update') }}/" + full.id + "\" class=\"hover:text-primary\" title=\"Update Purchases\"><i class=\"fi fi-rr-edit\"></i></a>&nbsp;&nbsp;";
                    $output += "<a href=\"javascript:void(0)\" onClick=\"removeItem('" + full.id + "', '" + full.name + "')\" class=\"hover:text-primary\" title=\"Remove Purchases\"><i class=\"fi fi-rr-trash\"></i></a>&nbsp;&nbsp;";
                    return $output;
                }},
            ]
        });
    ...
    function removeItem(id, name) {
        Swal.fire({
            title: 'Are you sure want to delete this purchases?',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('api.purchases.destroy') }}/" + id,
                    type: "DELETE",
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
                    success: function(response) {
                        Swal.fire({
                            title: 'Success!',
                            text: 'Purchases has been deleted!',
                            icon: 'success',
